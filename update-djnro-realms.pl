...
while (my $realm = $sth->fetchrow_hashref) {
    ...
    while (my $contact = $sth_c->fetchrow_hashref) {
        ...
        printf $nagConf "  alias                          %s\n", $contact->{'contact_name'} if exists $contact->{'contact_name'};
    ...
    printf $nagConf "  _EAP_ANONYMOUS                 %s\n", exists($c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'anonymous'}) ? $c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'anonymous'} : $realm->{'username'};
    printf $nagConf "  _EAP_USERNAME                  %s\n", exists($c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'username'}) ? $c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'username'} : $realm->{'username'};
    printf $nagConf "  _EAP_PASSWORD                  %s\n", exists($c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'pass'}) ? $c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'pass'} : $realm->{'pass'};
    print  $nagConf "}\n\n";
