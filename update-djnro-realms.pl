...
GetOptions($c,
    ...
    'tokenKey|key|k=s',
    'generateExtraService|extra|e:s',
    'write|w=s',
    'dump|D!',
    'help|h|?',
) or pod2usage(2);
...
while (my $realm = $sth->fetchrow_hashref) {
    next if grep { $_ eq $realm->{'realm'} } @{$c->{'excludeRealms'}};
    ...
    while (my $contact = $sth_c->fetchrow_hashref) {
        ...
        if (grep { $_ eq $contact->{'contact_email'} } @{$c->{disableContacts}}) {
    ...
    for my $service (qw/realm extra/) {
        my $suffix = '';
        if ($service eq 'extra') {
            next if not defined $c->{'generateExtraService'};
            $c->{'generateExtraService'} = 'Additional' unless $c->{'generateExtraService'};
            $suffix = '-extra';
        }

        printf $nagConf "# %s %s (instid_id = %d, realm_id = %d, instrealmmonid_id = %d, monlocauthpar_id = %d)\n", uc($service), $realm->{'realm'}, $realm->{'instid_id'}, $realm->{'realm_id'}, $realm->{'instrealmmonid_id'}, $realm->{'monlocauthpar_id'};
        print  $nagConf "# AUTOGENERATED - DO NOT EDIT!\n";
        ...
        printf $nagConf "  service_description            realm-%s%s\n", $defangedRealm, $suffix;
        printf $nagConf "  use                            djnro-generated-realm%s\n", $suffix;
        ...
        if (not exists($c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'disabled'}) and not $c->{'credentialOverride'}->{lc($realm->{'realm'})}->{'disabled'} and $realm->{'stage'} ne 'test') {
          printf $nagConf "  display_name                   Realm: %s%s\n", $realm->{'realm'}, $service eq 'extra' ? ' ('.$c->{'generateExtraService'}.')' : '';
        } else {
          printf $nagConf "  display_name                   Realm: %s%s [TEST]\n", $realm->{'realm'}, $service eq 'extra' ? ' ('.$c->{'generateExtraService'}.')' : '';
        ...
        printf $nagConf "# SERVICEDEPENDENCY %s%s\n", $realm->{'realm'}, $suffix;
        print  $nagConf "# AUTOGENERATED - DO NOT EDIT!\n";
        print  $nagConf "define servicedependency {\n";
        printf $nagConf "  use                            djnro-generated-servicedependency%s\n", $suffix;
        if ($service eq 'extra') {
            printf $nagConf "  service_description            realm-%s\n", $defangedRealm;
        }
        printf $nagConf "  dependent_service_description  realm-%s%s\n", $defangedRealm, $suffix;
        print  $nagConf "}\n\n";

        # Service escalations allow us to notify the service desk if nobody resolves the problem
        printf $nagConf "# SERVICEESCALATION %s%s\n", $realm->{'realm'}, $suffix;
        print  $nagConf "# AUTOGENERATED - DO NOT EDIT!\n";
        print  $nagConf "define serviceescalation {\n";
        printf $nagConf "  service_description            realm-%s%s\n", $defangedRealm, $suffix;
        printf $nagConf "  use                            djnro-generated-serviceescalation%s\n", $suffix;
        printf $nagConf "  contact_groups                 djnro-cg-%s\n", $defangedRealm;
        print  $nagConf "}\n\n";
    }
...
=item B<--generateExtraService>=C<string>, --extra, -e

Generate an additional service for monitoring e.g. best practices (i.e. two services per realm). The optional string is appended to the display name (defaults to C<Additional>).
...
=item B<excludeRealms>

A list of realms to completely exclude from monitoring.
