...
$sth = $dbh->prepare('SELECT realm,realm_id,name AS inst_name,instid_id,instrealmmonid_id,eap_method,phase2,username,pass FROM edumanage_instrealm LEFT JOIN edumanage_instrealmmon ON (edumanage_instrealm.id=realm_id) LEFT JOIN edumanage_monlocalauthnparam ON (edumanage_instrealmmon.id=instrealmmonid_id) LEFT JOIN edumanage_name_i18n ON (object_id=instid_id) WHERE mon_type = "localauthn" AND content_type_id=(SELECT id FROM django_content_type WHERE app_label = "edumanage" AND model="institution") AND lang="en" ORDER BY realm,eap_method,phase2;');
...
while (my $realm = $sth->fetchrow_hashref) {
    ...
    printf $nagConf "# REALM %s (instid_id = %d, realm_id = %d, instrealmmonid_id = %d)\n", $realm->{'realm'}, $realm->{'instid_id'}, $realm->{'realm_id'}, $realm->{'instrealmmonid_id'};
    print  $nagConf "# AUTOGENERATED - DO NOT EDIT!\n";
